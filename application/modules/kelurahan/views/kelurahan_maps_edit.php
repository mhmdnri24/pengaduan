...
<?php
    ...
    $lat = null;$lng=null;
    if($kelurahan->latlng!=null){
        $latlng = json_decode($kelurahan->latlng,true);
...
<script>
 ...
 var lat = "<?php echo $lat; ?>" || 118.0149;
 var lng = "<?php echo $lng; ?>" || -2.5489;
 var zoom = "<?php echo $lng; ?>" ?12 :5;
    
    var map = L.map('pikomap').setView([lng, lat ],zoom);
...
//   });
  ...
  var statesData;
  fetch('<?=base_url()?>kelurahan/getgejson/<?=$kelurahan->id_kelurahan?>')
    .then(response => response.json())
    .then(data => {
      if(data.features[0].geometry.coordinates[0][0] !=null){
          statesData = data;
