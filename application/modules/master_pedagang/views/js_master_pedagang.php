...
$(document).ready(function() {
    ...
    var table = $('#dataTable').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "<?= base_url('master_pedagang/ajax_data') ?>",
            "type": "POST",
            "data": function(d) {
    ...
    $('#btn-add').click(function() {
        resetForm();
        $('.modal-title').html('<i class="fa fa-plus"></i> Tambah Data Pedagang');
    ...
    $(document).on('click', '.btn-edit', function() {
        ...
        $.ajax({
            url: '<?= base_url('master_pedagang/get_by_id') ?>',
            type: 'POST',
            data: {
                id: id
            },
            ...
            success: function(response) {
                if (response.status) {
                    ...
                    $('.modal-title').html('<i class="fa fa-edit"></i> Edit Data Pedagang');
                    $('#modal-form').modal('show');
                } else {
    ...
    $(document).on('click', '.btn-sewa-detail', function() {
        ...
        // Redirect to penyewaan_blok module with pedagang_id parameter
        window.location.href = '<?= base_url('penyewaan_blok/index/') ?>' + id;
    });
    ...
    $(document).on('click', '.btn-delete', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('master_pedagang/delete') ?>',
                    type: 'POST',
                    data: {
                        id: id
                    },
    ...
    // Form submit
    $('#form-pedagang').submit(function(e) {
        e.preventDefault();

        var formData = new FormData(this);

        var submitBtn = $('#form-pedagang button[type="submit"]');
        var originalText = submitBtn.html();

        $.ajax({
            url: '<?= base_url('master_pedagang/save') ?>',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#id_kecamatan').change(function() {
        ...
        if (id_kecamatan) {
            $.ajax({
                url: '<?= base_url('master_pedagang/get_kelurahan') ?>',
                type: 'POST',
                data: {
...
function loadKecamatanOptions() {
    $.ajax({
        url: '<?= base_url('master_pedagang/get_kecamatan') ?>',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
